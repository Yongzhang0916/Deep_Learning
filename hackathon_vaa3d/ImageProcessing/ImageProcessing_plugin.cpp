...
//        get_sub_block(total1dData,in_sz,callback,out1dData);
    ...
    else if (func_name == tr("projection_swc"))
    {
        input_PARA PARA;
        projection_swc(callback,input,output,parent,PARA);
    }
    else if (func_name == tr("markers_distance"))
    {
        input_PARA PARA;
        markers_distance(callback,input,output,parent,PARA);
    }
    else if (func_name == tr("apo_to_markers"))
    {
        input_PARA PARA;
        apo_to_markers(callback,input,output,parent,PARA);
    }
